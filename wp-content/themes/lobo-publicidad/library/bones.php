...
function ted_scripts_and_styles()
{
    ...
    if ( !is_admin() ) {
        ...
        wp_register_script( 'ted-mixitup', get_stylesheet_directory_uri() . '/library/js/mixitup.min.js', array( 'jquery' ), '', true );
...
function ted_theme_support()
{
	...
	register_nav_menus(
		array(
            ...
            'home-nav'   => __( 'The Main Home', 'ted' ), // main nav home
...
function ted_related_posts()
{
	...
	if ( $tags ) {
		...
		if ( $related_posts ) {
            ...
            <?php endforeach ?>
		<?php } else { ?>
			<?php echo '<li class="no_related_post">' . __( 'No Related Posts Yet!', 'ted' ) . '</li>'; ?>
		<?php }
	}
